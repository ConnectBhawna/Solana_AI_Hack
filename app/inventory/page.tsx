...
export default function Products() {
  ...
  const updateStock = (id: string, newStock: number) => {
    // In a real application, you would update the stock on your Solana program here
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-2">
        <h2 className="text-3xl font-bold mb-6 text-center text-glow">Inventory Management</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product) => (
            <GlassCard key={product.id} className="glass-hover">
              <h3 className="text-xl font-semibold mb-2">{product.name}</h3>
              <p className="mb-1">Price: {product.price} SOL</p>
              <p className="mb-2">Current Stock: {product.stock}</p>
            </GlassCard>
          ))}
        </div>
      </div>
